...
int FastZoomBP::UseGraph(FactorGraph newGraph)
{
...
void FastZoomBP::GetState(vector< vector< FloatType > > & State)
{
	// State consists of:
	//  - EdgeToFn_Dist
	//  - EdgeToFn_DistEst
...
void FastZoomBP::RestoreState(vector< vector< FloatType > > const & State)
{
	int Index=0;
	for(auto & v:EdgeToFn_Dist)
	{
...
void * FastZoomBP::EdgeToFnThread(void * d)
{
...
void * FastZoomBP::EdgeToVarThread(void * d)
{
	// cast the input pointer so we can use it
	ThreadData * theData = (ThreadData *)d;
...
void * FastZoomBP::BeliefFromVarThread(void * d)
{
	// cast the input pointer so we can use it
	ThreadData * theData = (ThreadData *)d;
...
void FastZoomBP::IterThreads(int NumThreads)
{
	if(NumThreads==1)
	{
		Iter();
 ...
 *
 */
void FastZoomBP::UpdateMsgFromVar_Edge(int i)
{
	...
	///////////////////////////////////////////////////////
	FloatType MaxDiff = 0.0;
	for(int j=0;j<EdgeToFn_DistEst[i].size();j++)
	{
		FloatType tempDiff = abs( EdgeToFn_Dist[i][j] - EdgeToFn_DistEst[i][j] );
		if( MaxDiff < tempDiff )
		{
			MaxDiff = tempDiff;
			EdgeToFn_I[i] = j;
	...
	FloatType Projection = EdgeToFn_Dist[i][EdgeToFn_I[i]] - OldEst;
	if(QuantVarToFn)
	{
	...
	{
		EdgeToFn_DistEst[i][EdgeToFn_I[i]] += Projection ;
	}
 ...
 *
 */
void FastZoomBP::UpdateMsgFromFn_Edge(int i)
{
	// which function does the edge come from?
	int theFn=FnOfEdge[i];
	...
	for(int j=0;j<graph.functions[theFn].vars.size();j++)
	{
		if(j!=LeaveOut)
		{
			...
			EdgeToVar_Indices[i][j] = EdgeToFn_I[WhichIncomingEdge];
			...
			for(int k=0;k<graph.functions[theFn].ValuesSize / graph.variables[graph.functions[theFn].vars[j]].cardinality ; k++)
			{
				...
				EdgeToVar_Marginal[i][ EdgeToVar_Indices[i][LeaveOut] ] +=
					...
					* ProductOfInputs
					* EdgeToFn_Gamma[WhichIncomingEdge] ;
...

	{
		...
		{
			FloatType tempDiff = abs( EdgeToVar_Dist[i][j] - EdgeToVar_DistEst[i][j] );
			if( MaxDiff < tempDiff )
			{
				MaxDiff = tempDiff;
				EdgeToVar_I[i] = j;
		...
		FloatType Projection = EdgeToVar_Dist[i][EdgeToVar_I[i]] - OldEst;
		if(QuantFnToVar)
		{
		...
		{
			EdgeToVar_DistEst[i][EdgeToVar_I[i]] += Projection ;
		}
 ...
 *
 */
void FastZoomBP::UpdateBeliefFromVar_Edge(int i)
{
	// "clear" the message to all ones
	for(int k=0;k<EdgeToFn_NonNormal[ i ].size();k++)
		EdgeToFn_NonNormal[ i ] [ k ] = 1.0;
...
// DONE
vector<FloatType> FastZoomBP::MarginalOfVar(int WhichVar)
{
...
void FastZoomBP::MarginalOfVars(vector< vector<FloatType> > & Result)
{
...
void FastZoomBP::DecisionOfVars(vector< int > & Result)
{
	Result.resize(graph.variables.size());
	for(int i=0;i<graph.variables.size();i++)
	{
