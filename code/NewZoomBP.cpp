...
int NewZoomBP::UseGraph(FactorGraph newGraph)
{
...
void NewZoomBP::GetState(vector< vector< FloatType > > & State)
{
    // State consists of:
    //  - EdgeToFn_Dist
    //  - EdgeToFn_DistEst
...
void NewZoomBP::RestoreState(vector< vector< FloatType > > const & State)
{
    int Index=0;
    for(auto & v:EdgeToFn_Dist)
    {
...
void * NewZoomBP::EdgeToFnThread(void * d)
{
...
void * NewZoomBP::EdgeToVarThread(void * d)
{
    // cast the input pointer so we can use it
    ThreadData * theData = (ThreadData *)d;
...
void * NewZoomBP::BeliefFromVarThread(void * d)
{
    // cast the input pointer so we can use it
    ThreadData * theData = (ThreadData *)d;
...
void NewZoomBP::IterThreads(int NumThreads)
{
    if(NumThreads==1)
    {
        Iter();
 ...
 *
 */
void NewZoomBP::UpdateMsgFromVar_Edge(int i)
{
 ...
 *
 */
void NewZoomBP::UpdateMsgFromFn_Edge(int i)
{
    // which function does the edge come from?
    int theFn=FnOfEdge[i];
 ...
 *
 */
void NewZoomBP::UpdateBeliefFromVar_Edge(int i)
{
    // "clear" the message to all ones
    for(int k=0;k<EdgeToFn_NonNormal[ i ].size();k++)
        EdgeToFn_NonNormal[ i ] [ k ] = 1.0;
...
// DONE
vector<FloatType> NewZoomBP::MarginalOfVar(int WhichVar)
{
...
void NewZoomBP::MarginalOfVars(vector< vector<FloatType> > & Result)
{
...
void NewZoomBP::DecisionOfVars(vector< int > & Result)
{
    Result.resize(graph.variables.size());
    for(int i=0;i<graph.variables.size();i++)
    {
