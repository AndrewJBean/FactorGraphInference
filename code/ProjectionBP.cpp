...
string ProjectionBP::PrintParams()
{
	stringstream ss;
	ss << "KUpdate=" << KUpdate ;
	return ss.str();
...
int ProjectionBP::UseGraph(FactorGraph newGraph)
{
...
void ProjectionBP::GetState(vector< vector< FloatType > > & State)
{
	// State consists of:
	//  - EdgeToFn_Dist[][] 
	//  - EdgeToFn_DistEst[][]
...
void ProjectionBP::RestoreState(vector< vector< FloatType > > const & State)
{
	int Index=0;
	for(auto & v:EdgeToFn_Dist)
	{
...
///////////////////////
struct ProjectionBP::ThreadData
{
	ProjectionBP * Graph;
	int Progress;
...
void * ProjectionBP::EdgeToFnThread(void * d)
{
...
void * ProjectionBP::EdgeToVarThread(void * d)
{
	// cast the input pointer so we can use it
	ThreadData * theData = (ThreadData *)d;
...
void * ProjectionBP::BeliefFromVarThread(void * d)
{
	// cast the input pointer so we can use it
	ThreadData * theData = (ThreadData *)d;
...
void ProjectionBP::IterThreads(int NumThreads)
{
	if(NumThreads==1)
	{
		Iter();
...
void ProjectionBP::UpdateMsgFromVar_Edge(int i)
{
	///////////////////////////////////////////////////////////
 ...
 *
 */
void ProjectionBP::UpdateMsgFromFn_Edge(int i)
{
	// which function does the edge come from?
	int theFn=FnOfEdge[i];
 ...
 *
 */
void ProjectionBP::UpdateBeliefFromVar_Edge(int i)
{
	// "clear" the message to all ones
	for(int k=0;k<EdgeToFn_Dist[ i ].size();k++)
		EdgeToFn_Dist[ i ] [ k ] = 1.0;
...
// DONE
vector<FloatType> ProjectionBP::MarginalOfVar(int WhichVar)
{
...
void ProjectionBP::MarginalOfVars(vector< vector<FloatType> > & Result)
{
...
void ProjectionBP::DecisionOfVars(vector< int > & Result)
{
	Result.resize(graph.variables.size());
	for(int i=0;i<graph.variables.size();i++)
	{
...
void ProjectionBP::quick_select_largest(vector< FloatType > const & input, vector< int > & Scratch, int p, int r, int k)
{
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
int ProjectionBP::partition_largest(vector< FloatType > const & input, vector< int > & Scratch, int p, int r)
{
